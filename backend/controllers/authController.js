...
export const signup = catchAsync(async (req, res, next) => {
  ...
  if (!full_name || !email || !password || !confirmPassword) {
    return next(
      new appError("Please provide all information!", 400)
    );
  }
  ...
  if (password.length <= 7) {
    return next(
      new appError("Password is too short!", 400)
    );
  }
  ...
  if (founded) {
    return next(
      new appError("You already have an account", 409)
    );
  }
