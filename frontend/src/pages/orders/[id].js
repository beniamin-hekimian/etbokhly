...
export default function OrderDetailPage() {
  ...
  return (
    <>
      <Head>
        <title>{t.orders?.detail?.metaTitle}</title>
      </Head>
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
              ...
              <div className="flex flex-col gap-6 lg:col-span-2">
                ...
                <Card className="border-border bg-card shadow-sm">
                  <CardHeader className="pb-3">
                    <CardTitle className="text-lg font-bold">{t.orders?.detail?.orderInfo}</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        <span className="text-muted-foreground">{t.orders?.detail?.status}</span>
                        <div className="mt-1">
                      ...
                      <div>
                        <span className="text-muted-foreground">{t.orders?.detail?.date}</span>
                ...
                <Card className="border-border bg-card shadow-sm">
                  <CardHeader className="pb-3">
                    <CardTitle className="text-lg font-bold">{t.orders?.detail?.items}</CardTitle>
                  </CardHeader>
                        ...
                        <div className="flex flex-1 flex-col sm:flex-row sm:items-center sm:justify-between gap-1">
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground mt-0.5">
                              {t.orders?.detail?.priceEach}: {item.price} {t.latestMeals?.priceLabel}
                            </p>
                          </div>
              ...
              <div className="flex flex-col gap-6 lg:col-span-1">
                ...
                <Card className="border-border bg-card shadow-sm">
                  <CardContent className="pt-6">
                    <div className="flex items-center justify-between">
                      <span className="font-extrabold text-foreground">{t.orders?.detail?.total}</span>
                ...
                {order.chef && (
                  <Card className="border-border bg-card shadow-sm">
                    <CardHeader className="pb-3">
                      <CardTitle className="text-lg font-bold">{t.orders?.detail?.chefInfo}</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-bold text-foreground">{order.chef.full_name}</p>
                          {order.chef.location && (
